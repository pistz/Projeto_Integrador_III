...
export const notifyError = (error: unknown) => {
  if (axios.isAxiosError(error)) {
    ...
    const errorDetails: string = error.response?.data.aditional
      ? error.response?.data.aditional
      : null;
    console.error('\nMessage: ', errorMsg, ' \nAditional: ', errorDetails);
    const completeMessage =
      errorMsg && errorMsg.includes('Erro ao deletar')
        ? errorMsg.concat(' .Verifique o uso!')
        : errorMsg;
