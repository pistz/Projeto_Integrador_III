...
from src.domain.services.Stock.CurrentStock.current_stock_service_interface import (
    ICurrentStockService,
)
from src.infra.repositories.Products.products_repository_interface import (
    IProductsRepository,
)
from src.infra.repositories.Stock.CurrentStock.current_stock_repository_interface import (
    ICurrentStockRepository,
)
...
class CurrentStockService(ICurrentStockService):
    ...
    def __init__(self, current_stock_repository: ICurrentStockRepository):
    ...
    def get_full_current_stock(self) -> HttpResponse:
        current_list: list[CurrentStock] = (
            self.__current_stock_repository.get_full_current_stock()
        )
        current_stock_dto = [
            CurrentStockDTO(
    ...
    def __validate_product_existence(self, id: int) -> CurrentStockDTO:
        found_product = self.__current_stock_repository.get_current_stock_by_product_id(
            product_id=id
        )
